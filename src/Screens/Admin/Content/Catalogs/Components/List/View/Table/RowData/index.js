        ...
        <CellCheckbox key={0} className={clsx(styles.cell)}/>
        {
          displays.map(({type,style,...display},index)=>{
            switch (type) {
              case 'image':
                return (
                  <CellImage style={style} className={clsx(styles.cell)} display={display} key={index + 1} />
                )
              case 'checkbox':
                return (
                  <CellCheckbox style={style} className={clsx(styles.cell)} display={display} key={index + 1} />
                )
              case 'text':   
              default:
                return (
                  <CellText style={style} className={clsx(styles.cell)} display={display} key={index + 1} />
                )
            }
          })
        }
